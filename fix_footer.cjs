...
// 1. Revert the sidebar footer
const sidebarFooterRegex = /<div className="mt-auto p-4 border-t border-primary-900\/30 flex flex-col items-center gap-3">[\s\S]*?© Bara Official X Bara AI\s*<\/p>\s*<\/div>/;
const simpleFooter = `<div className="mt-auto p-4 text-center border-t border-primary-900/30">
                    <p className="text-xs text-gray-500 font-mono tracking-wider">
                      © Bara Official X Bara AI
                    </p>
                  </div>`;
content = content.replace(sidebarFooterRegex, simpleFooter);

// 2. Add "Ikuti Kami" to the Room Settings Modal
const roomSettingsFooter = `                  <button 
                    onClick={() => {
...
const deleteButtonRegex = /<button \s*onClick=\{\(\) => \{\s*setIsRoomSettingsOpen\(false\);\s*setIsDeleteRoomChatModalOpen\(true\);\s*\}\}\s*className="w-full py-3 rounded-xl border border-red-500\/30 text-red-400 hover:bg-red-500\/10 font-medium transition-colors flex items-center justify-center gap-2 cursor-pointer"\s*>\s*<Trash2 className="w-5 h-5" \/>\s*Hapus\s*<\/button>/;

content = content.replace(deleteButtonRegex, roomSettingsFooter);
